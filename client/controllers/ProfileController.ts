...
import { TextPosts, User, UserProfile } from "@prisma/client";
...
const FindUserTextPosts = async (
  userId: string
): Promise<TextPosts[] | null> => {
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}profile/findUserTextPosts/${userId}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
  ...
  const userTextPosts: TextPosts[] = await response.json();
  return userTextPosts;
};
